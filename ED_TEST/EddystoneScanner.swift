...
protocol EddystoneScannerDelegate {
    ...
    func didObserveURLBeacon(_ beaconScanner: EddystoneScanner, URL: URL, RSSI: Int)
}
...
class EddystoneScanner: NSObject, CBCentralManagerDelegate {
    ...
    ///
    func stopEddystoneScanning() {
        print("-- Prepares to stop Eddystone scanning. --")
        self.centralManager.stopScan()
        print("== Eddystone scanning ended. ==")
    }
            ...
            if eft == BeaconInfo.EddystoneFrameType.telemetryFrameType {
                ...
                if let
                    ...
                    if (self.seenEddystoneCache[beaconInfo.beaconID.description] != nil) {
                        ...
                        let onLostTimer = EddystoneDispatchTimer.scheduledDispatchTimer(
                            self.onLostTimeout,
                            queue: DispatchQueue.main) {
                                (timer: EddystoneDispatchTimer) -> () in
